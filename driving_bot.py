...
TERMS_PAGE_URL_FRAGMENT = "TermsAndConditions.xhtml" # Still useful for logging/confirmation if needed
TERMS_AGREE_CHECKBOX_SELECTOR = None 
...
def run_bot():
    ...
    with sync_playwright() as p:
        try:
            ...
            print("Clicked 'Continue' on Page 1. Waiting for Terms and Conditions page to become interactive...")
            # TERMS_CONTINUE_BUTTON_SELECTOR is "id=termsAndConditions:TermsAndConditionsForm:acceptButton"
            # This is the locator for the "Accept" button on the T&C page.
            accept_button_on_tc_page_locator = page.locator(TERMS_CONTINUE_BUTTON_SELECTOR)
            
            # Wait for the "Accept" button to be visible on the T&C page.
            # This implies the page has loaded enough for us to proceed.
            accept_button_on_tc_page_locator.wait_for(state="visible", timeout=30000) 
            # Optionally, also wait for it to be enabled if needed, though visible often suffices for click
            # accept_button_on_tc_page_locator.wait_for(state="enabled", timeout=10000) 
            print(f"Terms and Conditions page is ready (Accept button is visible). Current URL: {page.url}")
            # Verify we are indeed on the T&C page before clicking (optional sanity check)
            if TERMS_PAGE_URL_FRAGMENT not in page.url:
                print(f"Warning: Expected T&C page ('{TERMS_PAGE_URL_FRAGMENT}'), but current URL is '{page.url}'. Proceeding to click Accept button anyway.")


            # Checkbox interaction is skipped as TERMS_AGREE_CHECKBOX_SELECTOR is None
            if TERMS_AGREE_CHECKBOX_SELECTOR: # This block will be skipped
                # ... (checkbox logic, currently skipped) ...
                pass
            else:
                print("No 'I agree' checkbox selector defined for T&C page, skipping checkbox step.")

            # Click the "Accept" button on the T&C page (we've already waited for it)
            print(f"Clicking 'Accept' button on T&C page (selector: '{TERMS_CONTINUE_BUTTON_SELECTOR}')...")
            accept_button_on_tc_page_locator.click()
            ...
            for i, location in enumerate(LOCATIONS_TO_CHECK):
                ...
                if slots_found:
                    ...
                    message_parts = [f"*{current_location_name}* - Slots Found:"]
                    for slot in slots_found:
                        message_parts.append(f"  • _{slot}_")
